...
use crate::types::{Context, FilterMode, History, HistoryId, SearchMode, Settings};
use eyre::Result;
...
pub fn engine(search_mode: SearchMode, _settings: &Settings) -> Box<dyn SearchEngine> {
    match search_mode {
        ...
        SearchMode::DaemonFuzzy => {
            // Fall back to fuzzy mode (no daemon in this crate)
...
pub trait SearchEngine: Send + Sync + 'static {
    ...
    async fn query(&mut self, state: &SearchState, db: &mut dyn Db) -> Result<Vec<History>> {
