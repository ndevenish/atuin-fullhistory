...
use crate::types::{History, HistoryStats, Meaning, Settings, Theme, Timezone};
use ratatui::{
    Frame,
...
fn sort_duration_over_time(durations: &[(String, i64)]) -> Vec<(String, i64)> {
    ...
    let mut durations: Vec<(time::Date, i64)> = durations
        ...
        .map(|d| {
            (
                time::Date::parse(d.0.as_str(), &format).expect("invalid date string from db"),
                d.1,
            )
        })
        .collect();
    ...
    durations
        ...
        .map(|(date, duration)| {
            (
                date.format(output).expect("failed to format date"),
                *duration,
            )
        })
        .collect()
